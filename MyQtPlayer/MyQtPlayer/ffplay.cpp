...
static int wanted_stream[AVMEDIA_TYPE_NB] = {0};
...
static void video_image_display(VideoState *is)
{
    ...
    if (vp->bmp)
	{
		...
		SDL_RenderCopy(g_pRenderer, vp->bmp, NULL, &rect);
...
void alloc_picture(VideoState *is, AVFrame *src)
{
		...
		vp->height != src->height)
	{
		...
		vp->bmp = SDL_CreateTexture(g_pRenderer, SDL_PIXELFORMAT_IYUV,
   ...
   //     vp->height != src_frame->height) 
	{
		...
		if(g_ctl)
		{
			g_ctl->allocTexture(is, src_frame);
		}
...
static int stream_component_open(VideoState *is, int stream_index)
{
    ...
    case AVMEDIA_TYPE_AUDIO:
        ...
        is->audio_tid = SDL_CreateThread(audio_thread, "audio thread", is);
        SDL_PauseAudio(0);
        break;
    case AVMEDIA_TYPE_VIDEO:
...
//	ret = avformat_find_stream_info(ic, &options);
    ...
    if (!video_disable)
	{
        st_index[AVMEDIA_TYPE_VIDEO] = av_find_best_stream(ic, AVMEDIA_TYPE_VIDEO, 0, -1, NULL, 0);
	}
    if (!audio_disable)
	{
...
int createWin(void *winId)
{
	initSDL();
	if(winId)
	{
		g_pWindow = SDL_CreateWindowFrom(winId);
...
VideoState* play(char *fileName,  void *windId)
{
	if (!fileName)
	{
		NULL;
...
VideoState* ffplay(char *fileName,  void *windId)
{
	if(!gOperateLock)
	{
	...
	SDL_LockMutex(gOperateLock);
	VideoState *ret = play(fileName, windId);
